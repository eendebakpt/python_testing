...
# from pyperformance nbody, but now with numpy scalars
...
if 1:
    ...
    runner.timeit(name=f"np.abs({repr(i)})", stmt="np.abs(i)", setup=setup)
    runner.timeit(name=f"np.cos({repr(i)})", stmt="np.cos(i)", setup=setup)
    runner.timeit(name=f"np.abs({repr(f)})", stmt="np.abs(f)", setup=setup)
    runner.timeit(name=f"np.cos({repr(f)})", stmt="np.cos(f)", setup=setup)
if 1:
    runner.timeit(name="nbody", stmt="bench_nbody(10,  DEFAULT_REFERENCE, DEFAULT_ITERATIONS)", setup=setup)
